# CLAUDE.md — QueryClear Project Constitution

> This is the file Claude Code reads first. It is the source of truth for how to work in this repo.
> Read this fully at the start of every session, then read `memory.md` and `PROGRESS.md`.

## What we are building

QueryClear is an **autonomous SEO/AEO/GEO operator** for mid-market B2B SaaS companies. It is not a dashboard — it is an agent that *executes* the optimization work: monitoring brand visibility across AI engines, generating and publishing content, fixing on-page technical SEO, and running off-page citation campaigns.

The one-sentence positioning: **"Searchable tells your team what to do. QueryClear does it."**

Full strategy lives in `docs/strategic-brief.md`. Read it once for context, but the operational specs in `docs/` and `specs/` are what you build from.
...
## Repo layout (target — scaffold toward this)
...
│   └── db/                  # Prisma schema, migrations, row-level-security policies
├── docs/                    # Strategy + specs (read these)
├── specs/                   # Per-engine component specs (read these)
├── CLAUDE.md                # You are here
├── memory.md                # Session log — UPDATE AFTER EVERY SESSION
└── PROGRESS.md              # Milestone + task tracker — keep current
```

## Tech stack (committed defaults — see `docs/tech-stack.md` for rationale)
...
- **Billing:** Stripe — setup fees (one-time), subscriptions (tiered), and **usage-based overage** (metered tokens / published pieces).
- **Infra:** containerized agents; serverless-friendly (Modal/Daytona) so idle agents cost ~nothing.

If you deviate from these, record the decision and reason in `memory.md`.

## How to work in this repo

- **Start every session** by reading `CLAUDE.md` → `memory.md` → `PROGRESS.md`. Then pick up the next unblocked task in `PROGRESS.md`.
- **Build vertically, not horizontally.** Ship the thinnest end-to-end slice first — **start with Milestone 0 in `PROGRESS.md`**: one domain, one CMS (WordPress), Review mode only, OpenAI-only, publishing to **staging/draft, not a live site**. The M0 proof metric is *operator credibility* (does the loop reliably produce approved, non-breaking, audited, metered work?) — **not** visibility lift, which is too noisy on one domain. Prove the loop before breadth.
...
- Small PRs mapped to a single `PROGRESS.md` task ID.
- Secrets via env vars only; never commit keys. Provide `.env.example` whenever you add a new required var.
- Document every external integration in `docs/integrations.md` as you build it.

## End-of-session ritual (do this every time)

1. Update `memory.md` — what you did, key decisions, what's in progress, what's next, any gotchas.
2. Update `PROGRESS.md` — check off completed tasks, add newly discovered ones.
3. Leave the repo in a runnable state or clearly note in `memory.md` what's broken and why.

## Pointers

- Strategy & rationale → `docs/strategic-brief.md`
- Technical architecture → `docs/architecture.md`
- What the product does → `docs/product-spec.md`
- Concrete stack & infra → `docs/tech-stack.md`
- Data model & multi-tenancy → `docs/data-model.md`
- External integrations → `docs/integrations.md`
- Pricing, tiers, token budgets → `docs/pricing-and-tiers.md`
- Build phases → `docs/roadmap.md`
- Component specs → `specs/*.md`

---
Very Strict Guidelines!! Read and Understand.
