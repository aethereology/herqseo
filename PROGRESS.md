...
- [~] **M0-1** `AgentRuntime` interface + first real implementation for one domain. **Hermes dropped (D14, Session 32 — was never installed; the "HermesAgentRuntime" was always an in-memory stub).** **`ClaudeAgentRuntime` on the Claude Agent SDK is BUILT (Session 32)** — `claude_runtime.py`, offline-tested (metering, approval gate, 402, memory learnings), demo mode verified over HTTP. See `agent-runtime-layer.md` (was `hermes-agent-layer.md`). **Remaining:** one real-key e2e smoke — founder adds `ANTHROPIC_API_KEY` to `services/agent-runtime/.env`, `pip install -e ".[agent]"` (already done on this machine), start uvicorn, `POST /agents` then `POST /agents/{id}/run` with `payload={domain_url, brand}`.
...
- [~] Replace the in-memory agent placeholder with a real substrate. **Hermes dropped (D14); `ClaudeAgentRuntime` on the Claude Agent SDK BUILT (Session 32)** — `run()` = one SDK session with three LoopService tools (no publish tool), metered as one `agent_run` reservation (402 when unaffordable), manual trigger via `POST /agents/{id}/run`. Remaining: real-key e2e smoke (see M0-1). Later: scheduler, budget interrupt mid-run, DB-backed agent registry/memory, agent-runs dashboard.
