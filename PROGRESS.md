...
- [~] **M0-6** Thin internal UI to run the loop and approve. **Backend done:** `LoopService` (framework-free in-memory orchestration: run → review → publish, unit-tested) + thin `app.py` FastAPI adapter (`/runs`, `/drafts/{id}`, `/review`, `/publish`, `/audit`) exposing the TS↔Py boundary (D11). **Remaining:** Next.js pages in `apps/web` calling these endpoints + env-wired real service builder.
