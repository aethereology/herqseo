...
- [ ] **Audit latency:** a real-OpenAI audit is ~67s (was ~159s before the one-engine fix) — still slow for a self-serve sales tool. Sequential model calls are the cause. Proper fix = run the per-(prompt,sample) probes concurrently, but that requires a **thread-safe metering path** (reserve budget under a lock, call the model outside the lock, record usage under the lock) — touch the money path carefully and test it. Deferred from Session 29.
