...
- [x] **M0-1** `AgentRuntime` interface + first real implementation for one domain. **Hermes dropped (D14, Session 32 — was never installed; the "HermesAgentRuntime" was always an in-memory stub).** `ClaudeAgentRuntime` on the Claude Agent SDK is built (`claude_runtime.py`), offline-tested (metering, approval gate, 402, memory learnings), and **validated against a real Claude Agent SDK run (Session 33):** a live operator run against queryclear.com produced 25 opportunities + 1 draft + a genuinely coherent agent report/learnings in ~92s, was metered, and — critically — `POST /drafts/{id}/publish` still returned 409 for the agent's own real draft with no prior review. See `agent-runtime-layer.md` (was `hermes-agent-layer.md`).
...
- [x] **P1-8** Autonomy slider — "Review" mode only: human approval gate before publish. `content.assert_approved_for_publish(piece, autonomy_mode)` is the gate: it rejects any mode other than `"review"` and any piece not `approved`. `publish_content()` calls it FIRST (before any CMS write), and `LoopService.publish()` runs through `publish_content`. UI: `OperatorConsole` exposes approve/reject and the publish-before-approve path returns 409. Auto/Autopilot modes stay disabled until P2-5 (brand-safety guardrails). Tested in `test_content.py` (gate blocks unapproved/rejected/non-review, allows approved).
- [ ] **P1-9** Lift measurement: re-audit after 30 days, attribute change.
- [x] **P1-10** Minimal dashboard to show audit, opportunities, drafts, approvals, and lift. Dashboard (`apps/web` `/`) shows tenant/plan/mode/token metrics, the workflow steps, the live `OperatorConsole` (run → opportunities → draft → approve/reject → publish), guardrails, an **audit** entry point (`/audit`), a **Recent activity** panel (persistent approvals/publishes across runs, server-side `fetchAuditLog` → runtime `GET /audit-log`, degrades gracefully if the runtime is down), and an honest **Lift** placeholder (no number until P1-9 has a real before/after). Cross-run drafts/opportunities listing would need new list endpoints — deferred.
...
- [x] Replace the in-memory agent placeholder with a real substrate. **Hermes dropped (D14); `ClaudeAgentRuntime` on the Claude Agent SDK BUILT + validated live (Sessions 32–33)** — `run()` = one SDK session with three LoopService tools (no publish tool), metered as one `agent_run` reservation (402 when unaffordable), manual trigger via `POST /agents/{id}/run`. Later: scheduler, budget interrupt mid-run, DB-backed agent registry/memory, agent-runs dashboard.
...
- [x] **Audit latency:** fixed (Session 29). `TokenMeter` is now thread-safe (reserve `reserved_tokens` under a lock → model call outside the lock → record usage under the lock; reservation released on settle/error) so budgets stay enforced under concurrency, and `run_visibility_checks` runs probes through a `ThreadPoolExecutor` (`_MAX_PROBE_WORKERS=8`). Live audit ~159s → ~67s (one-engine) → **~17s** (concurrent). Money-path tested in `test_metering_concurrency.py` (no lost increments; reservation holds the cap under concurrency). Customer `run()` (5 engines × prompts × samples) benefits too. *Consideration:* 8 concurrent calls is conservative vs. OpenAI rate limits; provider has no 429 retry yet — revisit if high-volume runs hit limits.
