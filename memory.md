...
**Next concrete step (my rec):** **per-domain `BrandVoice`** (runs/audits currently use the default voice — wire the tenant's brand voice through `LoopService`). Then persistence (ContentPiece/Opportunity/AuditEvent → Postgres); then self-hosted WordPress publish validation. Hold Wix/WP.com OAuth + M0-1 Hermes. OpenAI key already in `services/agent-runtime/.env`.
...
- **Next:** per-domain `BrandVoice` through `LoopService` (currently always default voice).
