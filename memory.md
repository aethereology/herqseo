...
**Next concrete step (my rec):** **deepen technical checks** — add `llms.txt` + `robots.txt` presence, meta-description, and JSON-LD/schema detection (queryclear.com returned 0 findings because checks are still shallow; on-brand since llms.txt/schema is the pitch; needs crawler to capture head meta tags + a couple status-only fetches). Then per-domain `BrandVoice`; then persistence; then self-hosted WordPress publish validation. Hold Wix/WP.com OAuth + M0-1 Hermes. OpenAI key already in `services/agent-runtime/.env`.
